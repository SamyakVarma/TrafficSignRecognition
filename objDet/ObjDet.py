...
train_image_folder = 'objDetector/dataset/train/image/'
train_annot_folder = 'objDetector/dataset/train/annotation/gt.txt'
val_image_folder = 'objDetector/dataset/train/image/'
val_annot_folder = 'objDetector/dataset/train/annotation/gt.txt'
...
weight_reader = WeightReader('objDetector/yolov2.weights')
...
def parse_annotations(ann_file,img_dir,Labels):
    ...
    for line in annFile:
        ...
        if imgName in imgNames:
            ...
            boxes.append([])
            boxes[i].append([int(x_min),int(y_min),int(x_max),int(y_max),int(classID)])
...
def resizeImg(dataset):
    for batch in dataset:
        for i,img in enumerate(batch[0]):
            ...
            for ann in batch[1][i]:
                annOri=ann
                ann[0]=ann[0]-(shapeOri[0]-512)
                ann[1]=ann[1]-(shapeOri[1]-512)
                ann[2]=512*(annOri[2]-annOri[0])/shapeOri[0]
                ann[3]=512*(annOri[3]-annOri[1])/shapeOri[1]
    return(dataset)
...
def get_dataset(img_dir, ann_dir, labels, batch_size):
    ...
    '''
    imgs_name, bbox = parse_annotations(ann_dir, img_dir, LABELS)
...
model.save_weights('traffic.h5') #model weights saved
